...
void se_load_rom(const char *filename, int stateSlot){
  ...
  sb_breakup_path(filename,&base, &c, &ext);
    ...
    if(sb_path_has_file_ext(filename,".sav")){
      if(strncmp(filename,gui_state.recently_loaded_games[0].path,SB_FILE_PATH_SIZE)!=0){
        return se_load_rom(gui_state.recently_loaded_games[0].path, stateSlot);
      }
      return;
    }
  ...
  if (stateSlot > -1) {
    se_restore_state_slot(stateSlot);
  }
  return; 
}
...
static void se_draw_emulated_system_screen(){
  ...
  if(emu_state.system==SYSTEM_GBA && !emu_state.is_hidden){
...
void se_set_default_keybind(gui_state_t *gui){
  ...
  gui->key.bound_id[SE_KEY_A]     = SAPP_KEYCODE_INVALID;  
  gui->key.bound_id[SE_KEY_B]     = SAPP_KEYCODE_INVALID;
  gui->key.bound_id[SE_KEY_X]     = SAPP_KEYCODE_INVALID;
  gui->key.bound_id[SE_KEY_Y]     = SAPP_KEYCODE_INVALID;
  gui->key.bound_id[SE_KEY_UP]     = SAPP_KEYCODE_INVALID;  
  gui->key.bound_id[SE_KEY_DOWN]   = SAPP_KEYCODE_INVALID;    
  gui->key.bound_id[SE_KEY_LEFT]   = SAPP_KEYCODE_INVALID;    
  gui->key.bound_id[SE_KEY_RIGHT]  = SAPP_KEYCODE_INVALID;     
  gui->key.bound_id[SE_KEY_L]      = SAPP_KEYCODE_INVALID; 
  gui->key.bound_id[SE_KEY_R]      = SAPP_KEYCODE_INVALID; 
  gui->key.bound_id[SE_KEY_START]  = SAPP_KEYCODE_INVALID;      
  gui->key.bound_id[SE_KEY_SELECT] = SAPP_KEYCODE_INVALID; 
  gui->key.bound_id[SE_KEY_FOLD_SCREEN]= SAPP_KEYCODE_INVALID;     
  gui->key.bound_id[SE_KEY_PEN_DOWN]= SAPP_KEYCODE_INVALID; 
  gui->key.bound_id[SE_KEY_EMU_PAUSE]= SAPP_KEYCODE_INVALID;

  gui->key.bound_id[SE_KEY_EMU_PAUSE]= SAPP_KEYCODE_INVALID;     
  gui->key.bound_id[SE_KEY_EMU_REWIND]= SAPP_KEYCODE_INVALID;     
  gui->key.bound_id[SE_KEY_EMU_FF_2X]= SAPP_KEYCODE_INVALID;     
  gui->key.bound_id[SE_KEY_EMU_FF_MAX]= SAPP_KEYCODE_INVALID;     

  for(int i=0;i<SE_NUM_SAVE_STATES;++i){
    gui->key.bound_id[SE_KEY_CAPTURE_STATE(i)]=SAPP_KEYCODE_INVALID;
    gui->key.bound_id[SE_KEY_RESTORE_STATE(i)]=SAPP_KEYCODE_INVALID;
  }
...
void se_load_rom_overlay(bool visible){
  ...
  bool hover = false;
    ...
    //if(new_path[0])se_load_rom(new_path, -1);
    free(new_path);
...
static void se_poll_sdl(){
  ...
  emu_state.joy.inputs[SE_KEY_LEFT]   = js_left;
  emu_state.joy.inputs[SE_KEY_RIGHT]  = js_right;
  emu_state.joy.inputs[SE_KEY_UP]     = js_up;
  emu_state.joy.inputs[SE_KEY_DOWN]   = js_down;
  emu_state.joy.inputs[SE_KEY_A]      = js_a;
  emu_state.joy.inputs[SE_KEY_B]      = js_b;
  emu_state.joy.inputs[SE_KEY_L]      = js_l;
  emu_state.joy.inputs[SE_KEY_R]      = js_r;
  emu_state.joy.inputs[SE_KEY_START]  = js_start;
  emu_state.joy.inputs[SE_KEY_SELECT] = js_select;
...
static void frame(void) {
  ...
  if(emu_state.run_mode==SB_MODE_RUN||emu_state.run_mode==SB_MODE_REWIND)gui_state.overlay_open= false; 
...
static void init(void) {
  ...
  if(emu_state.cmd_line_arg_count>=2){
    printf("ReInit");
    se_load_rom(emu_state.cmd_line_args[1], -1);
  }
...
static void emsc_load_callback(const sapp_html5_fetch_response* response) {
  if (response->succeeded) {
    ...
    printf("Load Callback");
    se_load_rom((char*)response->user_data, -1);
  }else{
...
#else
        se_load_rom(sapp_get_dropped_file_path(0), -1);
#endif
    }
...
/* Exposed Action Keybinds */

void EMSCRIPTEN_KEEPALIVE RamWrite32(unsigned address, uint32_t data) {
  gba_store32(&core.gba, address, data);
}

uint32_t EMSCRIPTEN_KEEPALIVE ReadRam32(unsigned address) { 
  return gba_read32(&core.gba, address);
}

void EMSCRIPTEN_KEEPALIVE RamWrite16(unsigned address, uint32_t data) {
  gba_store16(&core.gba, address, data);
}

uint16_t EMSCRIPTEN_KEEPALIVE ReadRam16(unsigned address) { 
  return gba_read16(&core.gba, address);
}

void EMSCRIPTEN_KEEPALIVE RamWrite8(unsigned address, uint32_t data) {
  gba_store8(&core.gba, address, data);
}

uint8_t EMSCRIPTEN_KEEPALIVE ReadRam8(unsigned address) { 
  return gba_read8(&core.gba, address);
}

void EMSCRIPTEN_KEEPALIVE UpdateHour(uint8_t overrideHour) { 
  core.gba.rtc_override_hour = overrideHour;
}

void EMSCRIPTEN_KEEPALIVE SaveState(int slot) { 
  se_capture_state_slot(slot);
}

void EMSCRIPTEN_KEEPALIVE LoadState(int slot) { 
  se_restore_state_slot(slot);
}

void EMSCRIPTEN_KEEPALIVE SetGame(int game, int state){

  if (game == 0) {
    emu_state.game_number = 0;
    se_load_rom("/offline/FR.gba", state);
  } else if (game == 1) {
    emu_state.game_number = 1;
    se_load_rom("/offline/C.gba", state);
  } else {
    emu_state.game_number = 2;
    se_load_rom("/offline/E.gba", state);
  }

} 

void EMSCRIPTEN_KEEPALIVE ResetCore() {
  se_load_rom(gui_state.recently_loaded_games[0].path, -1);
}


void EMSCRIPTEN_KEEPALIVE HideGame() {
  emu_state.is_hidden = true;
}

void EMSCRIPTEN_KEEPALIVE ShowGame() {
  emu_state.is_hidden = false;
}


int EMSCRIPTEN_KEEPALIVE GetGame() {
...
void EMSCRIPTEN_KEEPALIVE SetSpeedUp(int amount){
  ...
  if (amount == 2) {
    emu_state.run_mode=SB_MODE_RUN;emu_state.step_frames=2;
  } else if (amount > 2){
...
int EMSCRIPTEN_KEEPALIVE GetSpeedUp(int amount){

  if (emu_state.step_frames == 2) {
    return 2;
...
void EMSCRIPTEN_KEEPALIVE SetVideoFilter(int filter){
    gui_state.settings.screen_shader=filter;
}

void EMSCRIPTEN_KEEPALIVE SetColorCorrection(int colorCorrection){
  gui_state.settings.color_correction=colorCorrection;
}


int EMSCRIPTEN_KEEPALIVE IncreaseVolume(){
  return gui_state.settings.volume += 0.1;
}

int EMSCRIPTEN_KEEPALIVE DecreaseVolume(){
  return gui_state.settings.volume -= 0.1;
}

void EMSCRIPTEN_KEEPALIVE Pause(){
  emu_state.run_mode=SB_MODE_PAUSE;emu_state.frame = 0; 
}
...
int EMSCRIPTEN_KEEPALIVE AKeyDown(){
  js_a = true;
  return emu_state.joy.inputs[SE_KEY_A] = 1;
}

int EMSCRIPTEN_KEEPALIVE AKeyUp(){
  js_a = false;
  return emu_state.joy.inputs[SE_KEY_A] = 0;
}

int EMSCRIPTEN_KEEPALIVE BKeyDown(){
  js_b = true;
  return emu_state.joy.inputs[SE_KEY_B] = 1;
}

int EMSCRIPTEN_KEEPALIVE BKeyUp(){
  js_b = false;
  return emu_state.joy.inputs[SE_KEY_B] = 0;
}

int EMSCRIPTEN_KEEPALIVE LKeyDown(){
  js_l = true;
  return emu_state.joy.inputs[SE_KEY_L] = 1;
}

int EMSCRIPTEN_KEEPALIVE LKeyUp(){
  js_l = false;
  return emu_state.joy.inputs[SE_KEY_L] = 0;
}

int EMSCRIPTEN_KEEPALIVE RKeyDown(){
  js_r = true;
  return emu_state.joy.inputs[SE_KEY_R] = 1;
}

int EMSCRIPTEN_KEEPALIVE RKeyUp(){
  js_r = false;
  return emu_state.joy.inputs[SE_KEY_R] = 0;
}

int EMSCRIPTEN_KEEPALIVE StartKeyDown(){
  js_start = true;
  return emu_state.joy.inputs[SE_KEY_START] = 1;
}

int EMSCRIPTEN_KEEPALIVE StartKeyUp(){
  js_start = false;
  return emu_state.joy.inputs[SE_KEY_START] = 0;
}

int EMSCRIPTEN_KEEPALIVE SelectKeyDown(){
  js_select = true;
  return emu_state.joy.inputs[SE_KEY_SELECT] = 1;
}

int EMSCRIPTEN_KEEPALIVE SelectKeyUp(){
 js_select = false;
  return emu_state.joy.inputs[SE_KEY_SELECT] = 0;
}

int EMSCRIPTEN_KEEPALIVE DLeftKeyDown(){
  js_left = true;
  return emu_state.joy.inputs[SE_KEY_LEFT] = 1;
}

int EMSCRIPTEN_KEEPALIVE DLeftKeyUp(){
  js_left = false;
  return emu_state.joy.inputs[SE_KEY_LEFT] = 0;
}

int EMSCRIPTEN_KEEPALIVE DRightKeyDown(){
  js_right = true;
  return emu_state.joy.inputs[SE_KEY_RIGHT] = 1;
}

int EMSCRIPTEN_KEEPALIVE DRightKeyUp(){
  js_right = false;
  return emu_state.joy.inputs[SE_KEY_RIGHT] = 0;
}

int EMSCRIPTEN_KEEPALIVE DUpKeyDown(){
  js_up = true;
  return emu_state.joy.inputs[SE_KEY_UP] = 1;
}

int EMSCRIPTEN_KEEPALIVE DUpKeyUp(){
  js_up = false;
  return emu_state.joy.inputs[SE_KEY_UP] = 0;
}

int EMSCRIPTEN_KEEPALIVE DDownKeyDown(){
  js_down = true;
  return emu_state.joy.inputs[SE_KEY_DOWN] = 1;
}

int EMSCRIPTEN_KEEPALIVE DDownKeyUp(){
  js_down = false;
  return emu_state.joy.inputs[SE_KEY_DOWN] = 0;
}
