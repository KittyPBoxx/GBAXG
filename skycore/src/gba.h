...
extern uint32_t externalCpuRead8Intercept(uint32_t address);
...
#define FIRE_RED_CURRENT_WARP 0x2031dbe
#define EMERALD_LAST_BANK     0x20322DC
#define EMERALD_CURRENT_BANK  0x20322e4
#define EMERALD_CURRENT_MAP   0x20322e5
#define EMERALD_CURRENT_WARP  0x20322e6
#define EMERALD_gMapHeader    0x2037318
#define FIRE_RED_gMapHeader   0x2036dfc

// static uint32_t read8hookAddresses[] = {FIRE_RED_CURRENT_BANK,
//                                         EMERALD_CURRENT_BANK};
// static uint32_t write8hookAddresses[] = {FIRE_RED_CURRENT_WARP, 
//                                          EMERALD_CURRENT_WARP};
// static uint32_t write32hookAddresses[] = {FIRE_RED_LAST_BANK, 
//                                           EMERALD_LAST_BANK,
//                                           FIRE_RED_gMapHeader,
//                                           EMERALD_gMapHeader};
...
static FORCE_INLINE void gba_process_rtc_state_machine(gba_t* gba){
  ...
  if(cs!=0){
    ...
    if(gba->rtc.serial_state==SERIAL_CLK_LOW&&clk){
      ...
      gba->mem.cart_rom[0x0000C4] = (gba->cart.gpio_data&~2)|(out_bit<<1);
    ...
    if(new_bit){
      ...
      if(gba->rtc.serial_bits_clocked==8) gba->rtc.state= SB_BFE(gba->rtc.input_register,0,4);
      int  cmd = SB_BFE(gba->rtc.state,1,3);
      bool read = SB_BFE(gba->rtc.state,0,1);
      switch(cmd){
        ...
        case RTC_STATUS:{
          ...
          if(gba->rtc.serial_bits_clocked==16){
            if(!read)gba->rtc.status_register=SB_BFE(gba->rtc.input_register,0,8);
            gba->rtc.state= RTC_RECV_CMD;
            gba->rtc.serial_bits_clocked=0;
          }
          break;
        ...
        case RTC_DATE_TIME:{
          if(gba->rtc.serial_bits_clocked==8) gba->rtc.output_register =
            ((uint64_t)(gba->rtc.year&0xff)       <<(0*8))|
            ((uint64_t)(gba->rtc.month&0xff)      <<(1*8))|
            ((uint64_t)(gba->rtc.day&0xff)        <<(2*8))|
            ((uint64_t)(gba->rtc.day_of_week&0xff)<<(3*8))|
            ((uint64_t)(gba->rtc.hour&0xff)       <<(4*8))|
            ((uint64_t)(gba->rtc.minute&0xff)     <<(5*8))|
            ((uint64_t)(gba->rtc.second&0xff)     <<(6*8));
...
static FORCE_INLINE uint8_t arm7_read8(void* user_data, uint32_t address){
  ...
  int game_number = ((gba_t*)user_data)->cart.game_number;
...
static FORCE_INLINE void arm7_write32(void* user_data, uint32_t address, uint32_t data){
  ...
  if ((address == FIRE_RED_LAST_BANK || address == FIRE_RED_gMapHeader) && game_number == 0) {
    bool preventWrite = externalCpuWrite32Intercept(address, data);
    if (preventWrite) {
      return;
    } 
  } else if ((address == EMERALD_CURRENT_BANK || address == EMERALD_gMapHeader) && game_number != 0) {
    bool preventWrite = externalCpuWrite32Intercept(address, data);
    if (preventWrite) {
      return;
    } 
...
static FORCE_INLINE void arm7_write8(void* user_data, uint32_t address, uint8_t data)  {
  ...
  if ((address == (FIRE_RED_CURRENT_BANK + 2) || address == FIRE_RED_gMapHeader) && game_number == 0) {
    bool preventWrite = externalCpuWrite8Intercept(address, data);
    if (preventWrite) {
      return;
    } 
  } else if ((address == (EMERALD_CURRENT_BANK + 2) || address == EMERALD_gMapHeader) && game_number != 0) {
    bool preventWrite = externalCpuWrite8Intercept(address, data);
    if (preventWrite) {
      return;
    } 
...
bool gba_load_rom(sb_emu_state_t*emu,gba_t* gba, gba_scratch_t *scratch){
  ...
  gba->cart.game_number = emu->game_number;
  ...
  if(!loaded_bios){
    ...
    scratch->loaded_bios=false;
  } else {
    scratch->skip_bios_intro=true;
    scratch->loaded_bios=true;
  }
...
void gba_tick_rtc(gba_t*gba){
  ...
  gba->rtc.hour  = gba->rtc_override_hour < 0 ? gba_bin_to_bcd(tm->tm_hour) : gba->rtc_override_hour;
